document.addEventListener("DOMContentLoaded", function () {
    ...
    window.addEventListener("load", clearEvents);
    ...
    closeButton.addEventListener("click", async function () {
        ...
        const sendResult = await sendEventsToServer(localEvents);

        const serverEvents = await fetchServerEvents();
        const tableHTML = generateEventTable(localEvents, serverEvents);
    ...
    async function sendEventsToServer(events) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error("Failed to send events to the server.");
            }
    ...
    function generateEventTable(localEvents, serverEvents) {
        ...
        for (let i = 0; i < maxRows; i++) {
            const localEvent = localEvents[i] ? `${localEvents[i].id}. ${localEvents[i].message} (${new Date(localEvents[i].time).toLocaleString()})` : "";
    ...
    async function clearEvents() {
        try {
            ...
            console.error("Event deleting error:", error);
        }
    }
});
